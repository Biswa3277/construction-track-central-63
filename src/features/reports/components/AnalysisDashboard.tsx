...
import { Download, Calendar, TrendingUp, TrendingDown, DollarSign, Users, Package, FileText } from "lucide-react";
...
const AnalysisDashboard = ({ reportType, onClose }: AnalysisDashboardProps) => {
  ...
  const generateAnalysisData = () => {
    ...
    const mockData = generateMockData(reportType, days);
  ...
  const exportReport = (format: string) => {
    toast.success(`Exporting ${reportType} analysis as ${format.toUpperCase()}...`);
  };
  ...
  const renderSummaryCards = () => {
    const cards = reportType === "Payment Summary" ? [
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">{reportType} Analysis</h2>
      ...
      {reportType === "Payment Summary" && renderPaymentSummaryCharts()}
      {reportType === "Project Expenditure" && renderProjectExpenditureCharts()}

      {!["Payment Summary", "Project Expenditure"].includes(reportType) && (
        <Card>
          <CardHeader>
            <CardTitle>Analysis Dashboard</CardTitle>
            <CardDescription>Detailed analysis for {reportType}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-center py-8">
              <p className="text-muted-foreground">Analysis dashboard for {reportType} is being prepared...</p>
            </div>
          </CardContent>
        </Card>
      )}
